...
@router.get("/weather")
async def get_weather(city: str = Query(..., description="City name (e.g., Bengaluru, Tokyo)")):
    """
    Get the current weather for a city. Don't forget your umbrella! ☔
    """
...
@router.get("/calc")
async def calculate_math(expression: str = Query(..., description="Math expression (e.g., 2+2*5)")):
    """
    Calculate basic math expressions safely. 
    We use the Math.js API to prevent code injection attacks! 🛡️
    """
    try:
        # We use an external API for safe evaluation so users can't run malicious Python code via eval()
...
async def generate_qr(data: str = Query(..., description="The URL or text to encode")):
    """
    Generates a high-quality QR code image URL for sharing links, notes, or contact info. 📸
    """
    # Using a fast, free public API for QR generation to keep our bot lightweight
